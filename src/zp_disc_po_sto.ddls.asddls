...
//                identification: [ { position: 20, label: 'STO Number', importance: #HIGH } ],
      ...
      @UI: {
             lineItem: [ { position: 70, label: 'Receiving plant', importance: #HIGH } ],
             fieldGroup: [ { qualifier: 'Fieldgroup2', label: 'Receiving plant', position: 40 } ] }
      
      receive_plant      as receive_plant,

      @UI: {
             lineItem: [ { position: 80, label: 'PO Quantity', importance: #MEDIUM } ],
      ...
      @UI: {
             lineItem: [ { position: 90, label: 'STO Quantity',importance: #MEDIUM } ],
    ...
    @UI: {
             lineItem:       [ { position: 100, label: 'PO start date', importance: #MEDIUM } ],
             fieldGroup:     [ { qualifier: 'Fieldgroup1', label: 'PO start date', position: 30 } ]  }
        
      po_start_date      as po_start_date,

      @UI: {
             lineItem:       [ { position: 115, label: 'PO end date', importance: #MEDIUM } ],
             fieldGroup:     [ { qualifier: 'Fieldgroup1', label: 'PO end date', position: 40 } ] }
        
      po_end_date        as po_end_date,
      
      @UI: {
             lineItem: [{ position: 130,  label: 'Disc days',importance: #MEDIUM, criticality: 'discrepancy_crit' } ],
      ...
      @UI: {
             lineItem: [ { position: 140, label: 'Disc reason',importance: #MEDIUM },
...
//      @Consumption.derivation: { lookupEntity: 'ZCDS_DISC_REASON_VALUES', resultElement: 'discrepancy_reason' , binding: [{targetParameter: 'ZDISC_REASON', type: #PARAMETER , value: 'p_domian_name' }]  }                        
      @Consumption.valueHelpDefinition: [{ entity: {name: 'ZCDS_DISC_REASON_VALUES' , element: 'value_low'  }, 
                                           distinctValues: true }]
      ...
      @UI: {
             lineItem: [{ position: 150,  label: 'Disc status',importance: #MEDIUM  },
      ...
      @Consumption.valueHelpDefinition: [{ entity: {name: 'ZCDS_DISC_STATUS_VALUES', element: 'value_low' },
                                           distinctValues: true }]  
      @UI.textArrangement: #TEXT_ONLY
      ...
      @UI: {
             lineItem:       [ { position: 170, label: 'Resolution text', importance: #LOW } ],
